...
void _main(void)
{
	...
	int32 envIdFib, envIdHelloWorld, helloWorldFrames;
	{
		...
		char garbage1 = user_arr[PAGE_SIZE*10-1] ;
		char garbage2 = user_arr[PAGE_SIZE*11-1] ;
		char garbage3 = user_arr[PAGE_SIZE*12-1] ;
		...
		*ptr3 = garbage1 ;
		...
		for (i = 0 ; i < PAGE_SIZE*4 ; i+=PAGE_SIZE/2)
		{
			...
			if (user_arr[i] != -1) panic("test failed!");
		}
		if (*ptr3 != user_arr[PAGE_SIZE*10-1]) panic("test failed!");
		...
		if (testCase ==3)
		{
			...
			if (*ptr4 != user_arr[PAGE_SIZE*11-1]) panic("test failed!");
